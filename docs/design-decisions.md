...
## DD-0035 · 「跑通且对占比已经很高 → DPO 收益预期小」是代码计算出的
  观察，不是编出来的判断

- 日期：2026-08-24 · Run: 无（无 GPU，本轮不产生真实 DPO 偏好对/训练
  数字；下方"实测数字"来自本轮为验证 `assess_expected_dpo_benefit`
  真实可用而手写的示例性合成数据，不是真实 SFT 模型的采样结果）
- 决策：`train/dpo.py::assess_expected_dpo_benefit` 把 PLAN.md 这条
  观察（"如果「跑通且对」占比已经很高，DPO 收益会很小——这个观察本身
  写进决策记录，比硬跑一遍有价值"）做成一个真实计算 `exec_ok_correct_
  share`（kept 候选里 EXEC_OK_CORRECT 占比）并按阈值（0.7，本项目自己
  定的、未经调参的判断线）返回可读结论的函数，而不是只在文档里写一句
  定性描述。
- 考虑过：只在本文件里写一句"如果准确率已经很高就不建议跑 DPO"，不
  写对应的代码。
- 为什么不这么做：PLAN.md 原文明确说这个观察"比硬跑一遍有价值"——
  言下之意是这条判断应该是训练流程真实会检查、真实会报告的一环，不是
  写完代码后另外补一句注释。把它做成 `PreferenceDatasetReport` 上的
  一个真实属性 + 一个真实的评估函数，意味着任何一次真实的 B4 偏好对
  构建（哪怕本沙箱里从未真的跑起来过）在写决策记录之前都会先有一个
  可引用的真实数字，而不是训练前凭印象猜一个"应该已经很高了吧"。
- 什么情况会失效：0.7 这个阈值本身没有实证支持（本项目没有做过
  "exec_ok_correct_share 到底多高时 DPO 实测增益趋近于零"这类消融），
  真机拿到第一批真实偏好对统计后，应该用真实数字复核这个阈值是否
  合理，而不是继续当作既定事实。
- 实测数字：下方脚本手写了 20 个问题、k=4、示例性 3:1
  正确:错误混合比例（不是任何真实模型的采样输出，纯粹用于验证函数
  可用）算出 `exec_ok_correct_share=75.0%`，触发"收益预期小"的判断，
